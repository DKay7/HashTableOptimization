...
static void HashTableCompileDump (const char* filename_dot, const char* filename_out, const uint64_t filename_size)
{
    ...
    sprintf (compile_cmd, "dot -Ksfdp -Tsvg %s -o %s", filename_dot, filename_out);
...
void HashTableDump (HashTable<K, V>* hash_table, void (*DataPrinter)(FILE*, HT_Pair<K, V>*), bool full_dump=false)
{
    ...
    {   
        ...
        __HashTableDrawFullSubgraph (file, hash_table);
    }

    else
    {
        __HashTableDrawMinSubgraph (file, hash_table, DataPrinter);
    }
...
template <typename K, typename V>
void __HashTableDrawFullSubgraph (FILE* file, HashTable<K, V>* hash_table)
{   
...
template <typename K, typename V>
void __HashTableDrawMinSubgraph (FILE* file, HashTable<K, V>* hash_table,  void (*DataPrinter)(FILE*, HT_Pair<K, V>*))
{   
        ...
        {               
            ...
            {   
                ...
                if (val_idx != hash_table->buckets[i].start_index)
                    fprintf(file, "list_node_%02lu:<prev_out> -> list_node_%02d:<prev_in>", 
                        val_idx, hash_table->values->list[val_idx].prev);

                if (bucket_idx != bucket_size - 1)
                    fprintf(file, "list_node_%02lu:<next_out> -> list_node_%02d:<next_in>", 
